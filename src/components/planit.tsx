...
import { CalendarDays, Copy, PanelsTopLeft, Plus, Presentation } from "lucide-react";
        ...
        <div className="flex flex-wrap items-center gap-1">
          ...
          <button
            ...
            onClick={() => {
              ...
              setNotice("Last week is on this week.");
            }}
          ...
          <button
            ...
            onClick={() => {
              ...
              setNotice("This week is on next week.");
            }}
  ...
  return (
    <section className="tw-gadget space-y-3 p-3" data-planit-hour>
      ...
      <div>
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          <button type="button" onClick={() => onEdit(copyYesterday(file, d, p))} className="tw-tap inline-flex min-h-11 items-center gap-1.5 rounded-xl bg-elevated px-3 text-sm font-semibold">
            <Copy className="size-3.5" aria-hidden />
            Same as last class day
          </button>
          <button type="button" onClick={() => onEdit(copyHourThroughWeek(file, d, p))} className="tw-tap min-h-11 rounded-xl bg-elevated px-3 text-sm font-semibold">
            Repeat through this week
          </button>
          {sameN ? (
            <button
              type="button"
              onClick={() => onEdit(copyHourToEmptySameGrade(file, d, p))}
              className="tw-tap min-h-11 rounded-xl bg-gold px-3 text-sm font-semibold text-bg"
            >
              Fill other G{cell.grade}
            </button>
          ) : null}
        </div>
      </div>
