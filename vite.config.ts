...
// @ts-expect-error JS plugin alongside the TS vite config
import { doorLinksPlugin } from "./scripts/door-links-plugin.mjs";
...
export default defineConfig(({ command, isPreview }) => {
  const onCf = Boolean(process.env.CF_PAGES) || process.env.NITRO_PRESET === "cloudflare_pages";
  return {
  server: {
    host: "0.0.0.0",
    port: 8080,
  ...
  plugins: [
    ...
    ...(command === "build" || isPreview
      ? [
          nitro({
            preset: onCf ? "cloudflare_pages" : "vercel",
            ...(onCf
              ? {
                  cloudflare: {
                    nodeCompat: true,
            ...
            routeRules: {
              ...
              "/tw.css": { headers: { "cache-control": "no-cache, must-revalidate" } },
